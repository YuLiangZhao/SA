...
public class TYImageView extends View {
	...
	public TYImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	public void revoke(){
        ...
        //Log.v(TAG, "撤销前有"+mPathList.size()+"条线");
    ...
    /**
     * 撤销全部（先将设置进来的图片覆盖历史背景，然后遍历线条集合，一条一条画进去）
     */
    public void clear(){
        if(currentStatus == STATUS_INIT) {//还未植入图片
            return;
	...
	public void recovery(){
		if(currentStatus == STATUS_INIT)//还未植入图片
			return;
		//Log.v(TAG, "恢复前有"+mPathList.size()+"条线");
		mCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);    //清空画布
        //画线
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			if(event.getPointerCount() ==1){
				...
				mDp = new DrawPath();
				mDp.path = mPath;
				//Log.v(TAG, "手指放下，当前线术"+mPathList.size());
		...
		case MotionEvent.ACTION_POINTER_UP:  //当第二根手指抬起
			if(event.getPointerCount() ==1){
				...
				if(hasLine){
	            	//Log.d(TAG, "保存有效线条");
	            	//Log.v(TAG, "手指抬起了，将线条放入集合，数量"+mPathList.size());
	...
	private void initBitmap(Canvas canvas) {
		if (sourceBitmap != null) {
			...
			Log.v(TAG, "图片的宽高"+bitmapWidth+", "+bitmapHeight+"，控件"+width+","+height);
			if (bitmapWidth > width || bitmapHeight > height) {
				if (bitmapWidth - width > bitmapHeight - height) {
					Log.v(TAG, bitmapWidth+"图片的宽度太大了"+width);
					// 当图片宽度大于屏幕宽度时，将图片等比例压缩，使它可以完全显示出来
					float ratio = width / (bitmapWidth * 1.0f);
					//Log.v(TAG, bitmapWidth+"图片的宽度太大了"+width+"，按照宽度缩放"+ratio);
					...
					Log.v(TAG, bitmapWidth+"图片的高度太大了"+width+"，按照高度缩放"+ratio);
					matrix.postScale(ratio, ratio);
				...
				Log.v(TAG, "图片的宽高都小于屏幕宽高"+"，X平移"+translateX+",Y平移"+translateY);
				matrix.postTranslate(translateX, translateY);
				totalTranslateX = translateX;
				totalTranslateY = translateY;
			...
			Log.v(TAG, "压缩后图片宽高"+currentBitmapWidth+" "+currentBitmapHeight);
			//创建一副原图大小相同的透明背景图片
...
//			bgBitmap = Bitmap.createBitmap(sourceBitmap,0,0,sourceBitmap.getWidth(),sourceBitmap.getHeight());
			...
			Log.v(TAG, "创建一个宽高为"+mCanvas.getWidth()+" "+mCanvas.getHeight()+"的画布");
